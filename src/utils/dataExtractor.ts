...
export const extractWebsiteData = (language: 'en' | 'fr' | 'es') => {
  ...
  const categorizedSkills = {
    frontend: uniqueTech.filter(tech => 
      ['React', 'TypeScript', 'Next.js', 'Tailwind CSS', 'HTML5 Canvas', 'WebGL'].includes(tech)
    ),
    backend: uniqueTech.filter(tech => 
      ['Node.js', 'Python', 'PostgreSQL', 'MongoDB', 'Redis', 'WebSockets'].includes(tech)
    ),
    cloud: uniqueTech.filter(tech => 
      ['AWS', 'Docker', 'Kubernetes', 'CI/CD'].includes(tech)
    ),
    tools: uniqueTech.filter(tech => 
      ['Git', 'VS Code', 'WebAssembly', 'REST APIs', 'Web Audio API', 'VS Code API', 'Rust'].includes(tech)
    )
  };
